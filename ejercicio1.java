...
public class ejercicio1 {
    public static void main(String[] args) {
        ...
        int n_nombres = Integer.parseInt(System.console().readLine("¿Cuántos nombres deseas generar? "));       //Le pedimos el numero de nombres que desea crear
        if (n_nombres <= 0) {
            System.out.println("Por favor, introduce un número válido de nombres a generar.");
            return;
        }
        String archivo = System.console()
                .readLine("Indica el nombre del archivo donde quieras guardar los nombres con la extensión: ");     //Le pedimos el nombre del archivo donde crear los nombres

        try {
            BufferedReader brNombres = new BufferedReader(new FileReader("ejercicio8_1/terminado/usa_nombres.txt"));            //lee los dos archivos
            ...
            for (int i = 0; i < n_nombres; i++) {
                String nombreCompleto = nombres[random.nextInt(nombres.length)] + " " + apellidos[random.nextInt(apellidos.length)];        //creamos los nombres de forma aleatoria
                bw.write(nombreCompleto + "\n");
            }
            System.out.println("Nombres generados con éxito.");

            brNombres.close();
            brApellidos.close();        //cerramos los buffered
            bw.close();
